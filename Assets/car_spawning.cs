    ...
    private void Start()
    {
        ...
        switch (Scoresave.getCar())
        {
            ...
            case Cars.KoenigseggJeskoAbsolute:
                player = Instantiate(car[1], spawnPosition, Quaternion.identity);
                CameraFollow.carTransform = player.transform;
                Enemy_spawner.Car = player.transform;
                break;

            case Cars.LamborghiniSianFKP37:
                player = Instantiate(car[2], spawnPosition, Quaternion.identity);
                CameraFollow.carTransform = player.transform;
                Enemy_spawner.Car = player.transform;
                break;

            case Cars.TeslaTruck:
                player = Instantiate(car[3], spawnPosition, Quaternion.identity);
                CameraFollow.carTransform = player.transform;
                Enemy_spawner.Car = player.transform;
                break;
